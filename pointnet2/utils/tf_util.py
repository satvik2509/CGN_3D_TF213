...
def batch_norm_template(inputs, is_training, scope, moments_dims_unused, bn_decay, data_format='NHWC'):
  """ Batch normalization compatible with TF 2.x (graph mode via compat.v1).
  
  Uses the TF1 compatibility layer so variable names remain identical to the
  names stored in the released checkpoints.
...
def dropout(inputs,
  ...
  with tf.variable_scope(scope) as sc:
    outputs = tf.cond(is_training,
                      lambda: tf.nn.dropout(inputs, keep_prob=keep_prob,
