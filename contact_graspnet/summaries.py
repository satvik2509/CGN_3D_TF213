...
def build_summary_ops(ops, sess, global_config):
    ...
    if global_config['MODEL']['bin_offsets']:
        ...
        tf.summary.scalar('offset_mean_abs_error', tf.reduce_mean(tf.abs(bin_mean_preds - tf.reshape(tf.abs(ops['offset_orig_labels_vals']), [-1]))))
        tf.summary.scalar('offset_max_abs_error', tf.reduce_max(tf.abs(bin_mean_preds - tf.reshape(tf.abs(ops['offset_orig_labels_vals']), [-1]))))

    merged = tf.summary.merge_all()

    acc_update_ops = top_grasp_acc_summaries(ops, thres=[0.62,0.65,0.68])

    binary_seg_flat = tf.reshape(ops['binary_seg_pred'], [-1])
    labels_flat = tf.reshape(tf.cast(ops['grasp_suc_labels_pc'], tf.bool), [-1])
    auc, auc_update_op = tf.compat.v1.metrics.auc(labels_flat, binary_seg_flat, curve='PR', summation_method='careful_interpolation')
    tf.summary.scalar('AUCPR', auc)

    try:
        from tensorboard import summary as summary_lib
        pr_curve_streaming_func = summary_lib.v1.pr_curve_streaming_op
        _, pr_update_op = pr_curve_streaming_func(name='pr_contact_success',
                                                    predictions=binary_seg_flat,
                                                    labels=labels_flat,
                                                    num_thresholds=201)
    except Exception:
        # Fallback: skip pr_curve if tensorboard API changed
        pr_update_op = tf.no_op(name='pr_update_noop')
