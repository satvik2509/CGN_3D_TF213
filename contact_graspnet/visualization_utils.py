...
def visualize_grasps(full_pc, pred_grasps_cam, scores, plot_opencv_cam=False, pc_colors=None, gripper_openings=None, gripper_width=0.08):
    ...
    if os.environ.get('QT_QPA_PLATFORM') == 'offscreen':
        ...
        mlab.savefig('visualisation/3d_grasps.png')
        print("Saved 3D visualization to visualisation/3d_grasps.png (headless mode)")
    else:
        try:
            mlab.show()
        except Exception as e:
            print("Failed to show interactive window:", e)
            mlab.options.offscreen = True
            mlab.savefig('visualisation/3d_grasps.png')
            print("Saved 3D visualization to visualisation/3d_grasps.png as fallback.")
