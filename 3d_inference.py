...
# ----------------------------------------------------------------------------
# Fixed cam -> flange (TCP) calibration
# ----------------------------------------------------------------------------
T_CAM_TCP = np.array([
    [ 0.999997,  0.002611,  0.000299, -0.032097],
    [-0.002611,  0.999996,  0.000547, -0.094656],
    [-0.000297, -0.000548,  1.000000,  0.068285],
    [ 0.000000,  0.000000,  0.000000,  1.000000],
], dtype=float)


def get_tcp_base_from_robodk(robot_name='UR7e'):
    """
    Pulls the robot's current flange(TCP)->base pose live from RoboDK and
    returns it as a 4x4 numpy array.
    """
    RDK = Robolink()
    robot = RDK.Item(robot_name)
    if not robot.Valid():
        raise RuntimeError(f"RoboDK: robot item '{robot_name}' not found in the open station.")
    pose = robot.Pose()  # robodk.robomath.Mat, base->flange
    T_tcp_base = np.array(pose.Rows())
    return T_tcp_base
...
def process_ply_scene(sess, grasp_estimator, ply_path, eps=0.02, min_points=30,
    ...
    # --- move everything from the (shifted) camera frame into the robot base frame ---
    if T_cam_base is not None:
        R_cb, t_cb = T_cam_base[:3, :3], T_cam_base[:3, 3]

        full_points = (R_cb @ full_points.T).T + t_cb
        if table_inlier_points is not None:
            table_inlier_points = (R_cb @ table_inlier_points.T).T + t_cb

        for obj_id in pred_grasps_cam:
            if len(pred_grasps_cam[obj_id]):
                pred_grasps_cam[obj_id] = pred_grasps_cam[obj_id].copy()
                pred_grasps_cam[obj_id] = np.einsum('ij,njk->nik', T_cam_base, pred_grasps_cam[obj_id])
            if contact_pairs.get(obj_id) is not None and len(contact_pairs[obj_id]):
                cp = contact_pairs[obj_id]
                contact_pairs[obj_id] = (R_cb @ cp.reshape(-1, 3).T).T.reshape(cp.shape) + t_cb

        print("Applied cam->base transform on top of the z=0 shift; scene now expressed in robot base frame.")
...
def main():
    ...
    parser.add_argument('--robot_name', type=str, default='UR7e',
                         help='RoboDK station item name of the robot, used to fetch the live flange->base pose')
    parser.add_argument('--no_base_frame', action='store_true',
                         help='Skip the cam->base transform entirely and keep output in the shifted camera frame')
    ...
    T_cam_base = None
    if not args.no_base_frame:
        T_tcp_base = get_tcp_base_from_robodk(args.robot_name)
        T_cam_base = T_tcp_base @ T_CAM_TCP
        print(f"Fetched flange->base pose from RoboDK item '{args.robot_name}':\n{T_tcp_base}")
        print(f"Resulting cam->base transform:\n{T_cam_base}")
